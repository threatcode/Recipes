...
# Threat Linux ISO recipe for : Top 10 Mate non-root
#########################################################################################
# Desktop 	: Mate 1.8
# Metapackages	: threat-linux-top10
# ISO size 	: 1.36 GB 
...
# Background	: http://www.offensive-security.com/threat-linux/threat-linux-recipes/
#########################################################################################
...
# Clone the default Threat live-build config.

git clone https://github.com/threatcode/build-scripts/live-build-config.git

# Get the source package of the debian installer. 
# The default Threat preseed file lives here, and will need changing for non-root user support.
...
# The user doesn't need the threat-linux-full metapackage, we overwrite with our own basic packages.
# This includes the debian-installer and the threat-linux-top10 metapackage (commented out for brevity of build, uncomment if needed).

cat > config/package-lists/threat.list.chroot << EOF
threat-root-login
threat-defaults
threat-menu
threat-archive-keyring
...
mkdir -p config/includes.chroot/usr/share/wallpapers/threat/contents/images
wget http://1hdwallpapers.com/wallpapers/threat_linux.jpg
mv threat_linux.jpg config/includes.chroot/usr/share/wallpapers/threat/contents/images
...
dbus-launch --exit-with-session gsettings set org.mate.background picture-filename '/usr/share/wallpapers/threat/contents/images/threat_linux.jpg'
...
# We modify the default Threat preseed which disables normal user creation. 
